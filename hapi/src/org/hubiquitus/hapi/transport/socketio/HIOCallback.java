...
//			}
			...
			if(arg0.equals("attrs") && response != null){
				try {
					...
					Log.i(getClass().getCanonicalName(),"JSON exception while getting attributes");
...
//				Log.i(getClass().getCanonicalName(), "Attributes : " + callback.getAttributes().toString());
			...
			// stopping the timer
//			try {
//				if(arg0.equals("link") && response.getString("status").equals("connected")){
//					
//					if(!HTimer.isTimedOut()){
//						Log.e(getClass().getCanonicalName(), "pas time out");
//					}
//					else{
//						Log.e(getClass().getCanonicalName(), "time out");
//						HTimer hTimer = HTimer.getHTimer(null);
//						callback.hCallback(Context.LINK, Status.ERROR, new JSONObject("{\"status\":\"disconnected\",\"code\":" + Error.CONNECTION_TIMEOUT + "}"));
//						
//						
//					}
//				}
//			} catch (JSONException e) {
//				Log.i(getClass().getCanonicalName(),"JSON exception while getting status");
//				Log.i(getClass().getCanonicalName(), e.getMessage());
//			}
